...
class ProposalTargetCreator(object):
    ...
    def __init__(
        ...
        self.neg_iou_thresh_lo = neg_iou_thresh_lo  # Default 0.1 in py-faster-rcnn
...
class AnchorTargetCreator(object):
    ...
    def _calc_ious(self, anchor, bbox, inside_index):
        """
        Calculate IoU (Intersection over Union) between anchors and ground truth bounding boxes.

        Args:
            anchor (numpy.ndarray): Array of anchor boxes.
            bbox (numpy.ndarray): Array of ground truth bounding boxes.
            inside_index (numpy.ndarray): Indices of anchors inside the image.

        Returns:
            tuple: A tuple containing:
                - argmax_ious (numpy.ndarray): Indices of maximum IoU ground truth boxes for each anchor.
                - max_ious (numpy.ndarray): Maximum IoU value for each anchor.
                - gt_argmax_ious (numpy.ndarray): Indices of anchors that have the highest IoU for each ground truth box.
        """
        # Calculate IoUs between the anchors and the ground truth boxes
...
class ProposalCreator(object):
    ...
    def __call__(self, loc, score, anchor, img_size, scale=1.0):
        ...
        # NOTE  Convert anchors into proposal via bbox transformations.
